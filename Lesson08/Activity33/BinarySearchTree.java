...
/**
* This is the method exposed as public for adding elements into the
Tree.
* it checks if the size == 0 and then adds the element into parent
node. if
* parent is already filled,creates a New Node with data and calls
the
...
/**
* Takes two params, root node and newNode. As per BST, check if the
root
...
* added in right (again recursively check if it has child nodes and
the
...
root:
/**
* As per BST, the left most node will be lowest of the all.
iterate all the
* left nodes until we reach empty left and get the value of it
root.
...
/**
* As per BST, the right most node will be highest of the all.
iterate all
* the right nodes until we reach empty right and get the value of
it root.
